# OCR Gateway E2E 跑前校验：GitLab + Runner + Gateway（默认跳过 AICR :8001）
param(
...
# scripts/acceptance -> ocr-ci2 根
$OcrCi2Root = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
# 仅当上级目录含 evn/ 时视为 monorepo（独立 checkout 不臆测路径）
$DefaultMonorepoRoot = Split-Path -Parent $OcrCi2Root
$MonorepoRoot = if ($env:OCR_MONOREPO_ROOT) {
    $env:OCR_MONOREPO_ROOT
} elseif (Test-Path (Join-Path $DefaultMonorepoRoot "evn")) {
    $DefaultMonorepoRoot
} else {
    $null
}
$RunnerConfig = if ($env:OCR_RUNNER_CONFIG) {
    $env:OCR_RUNNER_CONFIG
} elseif ($MonorepoRoot) {
    Join-Path $MonorepoRoot "evn\gitlab-runner\config\config.toml"
} else {
    ""
}
$TokenHelper = Join-Path $OcrCi2Root "scripts\acceptance\print_gitlab_token.py"

# 默认仅 Gateway；显式 -IncludeAicr 时检查 AICR
$SkipAicr = $OcrGatewayOnly -or (-not $IncludeAicr)

function Get-GitLabToken {
    param([switch]$PreferAdmin)
    $order = if ($PreferAdmin) { @("ROOT_PAT", "AICR_BOT_TOKEN", "GITLAB_API_TOKEN") } else { @("AICR_BOT_TOKEN", "ROOT_PAT", "GITLAB_API_TOKEN") }
    foreach ($name in $order) {
        $v = [Environment]::GetEnvironmentVariable($name, "Process")
        if ($v) { return $v }
    }
    if (-not $EnvFile -and $env:OCR_CI2_ENV_FILE) { $EnvFile = $env:OCR_CI2_ENV_FILE }
    $candidates = @()
    if ($EnvFile) { $candidates += $EnvFile }
    $candidates += Join-Path $OcrCi2Root ".env"
    if ($MonorepoRoot) { $candidates += Join-Path $MonorepoRoot "evn\.env" }
    foreach ($envPath in $candidates) {
        if (-not $envPath -or -not (Test-Path $envPath)) { continue }
        $found = @{}
        foreach ($line in Get-Content $envPath) {
            if ($line -match '^\s*(ROOT_PAT|AICR_BOT_TOKEN|GITLAB_API_TOKEN)\s*=\s*(.+)\s*$') {
    ...
    # 与 Python env_loader 一致：回退 ~/.opencodereview/config.json
    if ((Test-Path $TokenHelper)) {
        try {
            $pyToken = & python $TokenHelper 2>$null
            if ($LASTEXITCODE -eq 0 -and $pyToken) { return $pyToken.Trim() }
        } catch { }
    }
    return $null
}
...
try {
    ...
    if ($_.Exception.Response.StatusCode.value__ -in 302, 301) {
        ...
        Add-Check "gitlab" $false $_.Exception.Message "Start GitLab (e.g. monorepo evn/gitlab/start.ps1 or company compose)"
    }
}

# GitLab container
$gitlabPs = docker ps --filter "name=^gitlab$" --format "{{.Status}}" 2>$null
Add-Check "gitlab_container" ([bool]$gitlabPs) ($(if ($gitlabPs) { $gitlabPs } else { "not running" })) "evn/gitlab/start.ps1 or OCR_GITLAB_COMPOSE_DIR"
...
# config.toml
$cfgOk = $RunnerConfig -and (Test-Path $RunnerConfig)
$runnerFix = if ($MonorepoRoot) {
    "Set OCR_RUNNER_CONFIG or register runner; see evn/gitlab/README.md"
} else {
    "Set OCR_RUNNER_CONFIG to your gitlab-runner config.toml (standalone checkout)"
}
Add-Check "runner_config" $cfgOk $(if ($cfgOk) { $RunnerConfig } else { "not found" }) $runnerFix
...
# AICR on host（仅 -IncludeAicr 且未 -OcrGatewayOnly）
if (-not $SkipAicr) {
    try {
        ...
        Add-Check "aicr_host" $false "AICR not reachable at $AicrUrl" "Start AICR on :8001 or use -OcrGatewayOnly"
    }
}
...
# GitLab CI variable OCR_GATEWAY_SECRET
...
if ($null -eq $secretCheck) {
    Add-Check "ocr_gateway_secret_var" $false "cannot query CI variables for $ProjectPath (need ROOT_PAT / AICR_BOT_TOKEN / config.json)" `
...
if ($Json) {
    ...
    Write-Host "=== OCR Gateway preflight (ocr-ci2) ===" -ForegroundColor Cyan
    foreach ($c in $checks) {
    ...
    Write-Host ""
    if ($report.ok) {
        Write-Host "All checks passed. Ready for OCR Gateway E2E." -ForegroundColor Green
    } else {
        Write-Host "$($failed.Count) check(s) need attention before E2E." -ForegroundColor Yellow
    }
}
