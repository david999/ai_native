...
from lib.paths import OCR_CI2_ROOT, get_results_root
...
VERIFY_SCRIPT = OCR_CI2_ROOT / "scripts" / "acceptance" / "verify_gateway_runner.ps1"
CREATE_MR_SCRIPT = OCR_CI2_ROOT / "scripts" / "acceptance" / "create_or_update_mr.py"
...
def run_scenario(scenario_id: str, *, skip_preflight: bool) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ocr-e2e-") as tmp:
        ...
        out_dir = get_results_root() / f"ocr-e2e-{ts}-{scenario_id}"
        collect_report(
